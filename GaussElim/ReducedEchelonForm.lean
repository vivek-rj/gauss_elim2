...
| cons : (row : ReducedRow n) → RowReducedEchelonList R → (∀ r ∈ R, (hz : r.z.isSome) → row.toVector.get (r.z.get hz) = 0) → ((hz : row.z.isSome) → (R.map fun r => r.toVector.get (row.z.get hz)).all (fun x => x=0)) → RowReducedEchelonList (R.concat row)
...
def m1 : RowReducedEchelonForm 4 4 [a1,a2,a3,a4] :=
  ⟨RowReducedEchelonList.cons a4 (RowReducedEchelonList.cons a3 (RowReducedEchelonList.cons a2 (RowReducedEchelonList.cons a1 RowReducedEchelonList.nil (by intro r hr1 hr2; simp at hr1) (by intro hz; simp)) (by intro r hr1 hr2; simp at hr1; simp [hr1,a2,a1,ReducedRow.toVector,zeroVec,Vector.replicate,Vector.append,Vector.head]) (by intro hz; simp [a1,a2,ReducedRow.toVector,zeroVec,Vector.replicate,Vector.append,Vector.get])) (by intro r hr1 hr2; simp at hr1; rcases hr1 with hr1|hr1 <;> simp [hr1,a3,a2,a1,ReducedRow.toVector,zeroVec,Vector.replicate,Vector.append,Vector.head,Vector.get]) (by intro hz; simp [a1,a2,a3,ReducedRow.toVector,zeroVec,Vector.replicate,Vector.append,Vector.get,List.get])) (by intro r hr1 hr2; simp at hr1; rcases hr1 with hr1|hr1|hr1 <;> simp [hr1,a4,a3,a2,a1,ReducedRow.toVector,zeroVec,Vector.replicate,Vector.append,Vector.head,Vector.get,List.get]) (by intro hz; simp [a4] at hz),rfl⟩

#eval m1.toMatrix

def m2 : RowReducedEchelonForm 4 4 [a1,a2,a4,a3] :=
  ⟨RowReducedEchelonList.cons a3 (RowReducedEchelonList.cons a4 (RowReducedEchelonList.cons a2 (RowReducedEchelonList.cons a1 RowReducedEchelonList.nil (by intro r hr1 hr2; simp at hr1) (by intro hz; simp)) (by intro r hr1 hr2; simp at hr1; simp [hr1,a2,a1,ReducedRow.toVector,zeroVec,Vector.replicate,Vector.append,Vector.head]) (by intro hz; simp [a1,a2,ReducedRow.toVector,zeroVec,Vector.replicate,Vector.append,Vector.get])) (by intro r hr1 hr2; simp at hr1; rcases hr1 with hr1|hr1 <;> simp [hr1,a4,a2,a1,ReducedRow.toVector,zeroVec,Vector.replicate,Vector.append,Vector.head,Vector.get]) (by intro hz; simp [a4] at hz)) (by intro r hr1 hr2; simp at hr1; rcases hr1 with hr1|hr1|hr1; simp [hr1,a4,a3,a2,a1,ReducedRow.toVector,zeroVec,Vector.replicate,Vector.append,Vector.head,Vector.get,List.get]; simp [hr1,a4,a3,a2,a1,ReducedRow.toVector,zeroVec,Vector.replicate,Vector.append,Vector.head,Vector.get,List.get]; simp [hr1,a4] at hr2) (by intro hz; simp [a1,a2,a4,ReducedRow.toVector,zeroVec,Vector.replicate,Vector.append,Vector.get,List.get]),rfl⟩

#eval m2.toMatrix --not supposed to happen
